Meteor.methods({
    ...
    footballAdd: function(marketType, target, valuations, footballType, currentFootballId) {
        ...
        var currentActivated = Footballs.findOne({_id:currentFootballId}).footballActivated;
    ...
    footballValuationsExpand: function(marketType, selections) {
        ...
        if(marketType === currentMarketType) {
            ...
            Footballs.update({_id:currentFootballId},{$addToSet:{footballValuations:{$each:selectionsFinal}}});
            var activated = Footballs.findOne({_id:currentFootballId}).footballActivated;
            if(activated == false) {
                Footballs.update({_id:currentFootballId},{$set:{footballActivated:true}});
            }
        } else {
    ...
    footballValuationsNew: function(marketType, selections) {
        ...
        Footballs.update({_id:newFootballId},{$addToSet:{footballValuations:{$each:selectionsFinal}}});
        var activated = Footballs.findOne({_id:newFootballId}).footballActivated;
        if(activated == false) {
            Footballs.update({_id:newFootballId},{$set:{footballActivated:true}});
        }
                ...
                if(targetSelection !== "") {
                    switch(marketType) {
                        case "company":
                            ...
                            break;
                        case "team":
                            var teamId = FeedTeams.findOne({teamName:targetSelection})._id;
                            return {
                                targetId: teamId,
                                targetType: marketType,
                                targetData: "feed"
    ...
    footballTargetUpdateAndCopy: function(currentFootballId, targetObject, footballType, saveCopy) {
        ...
        if(saveCopy) {
            ...
            console.log(currentFootballExtended);
    ...
    footballCurrentToggle: function(currentFootballId) {
        ...
        switch(currentCurrent) {
            case true:
                ...
                if(trading == false && valuationsCount == 0) {
                    Footballs.update(currentFootballId, {$set: {includeCurrent:false, footballActivated: false}});
                } else {
                    ...
                    if(trading == false && valuationsCount == 1 && selectionsCount == 0) {
                        Footballs.update(currentFootballId, {$set: {includeCurrent:false, footballActivated: false}});
                    } else {
                ...
                break;
            case false:
                Footballs.update(currentFootballId, {$set: {includeCurrent:true, footballActivated: true}});
                break;
        }
    },
    ...
    footballTradingToggle: function(currentFootballId) {
        ...
        switch(currentTrading) {
            case true:
                ...
                var valuations = Footballs.findOne({_id:currentFootballId}).footballValuations;
                var valuationsCount = valuations.length;
                if(current == false && valuationsCount == 0) {
                    Footballs.update(currentFootballId, {$set: {includeTrading:false, footballActivated: false}});
                } else {
                    var valuationId = valuations[0];
                    var selections = Valuations.findOne({_id:valuationId}).valuationSelections;
                    var selectionsCount = selections.length;
                    if(current == false && valuationsCount == 1 && selectionsCount == 0) {
                        Footballs.update(currentFootballId, {$set: {includeTrading:false, footballActivated: false}});
                    } else {
                ...
                break;
            case false:
                Footballs.update(currentFootballId, {$set: {includeTrading:true, footballActivated: true}});
                break;
        }
    },
    //Add new valuation with selected comps or deals in Gallery
    valuationAdd: function(marketType, type, element, metric, period, output, outputPeriod, selections, currentFootballId, activated) {
        check(marketType, String);
        check(type, String);
        ...
        var getValuation = function(valuation) {
            var currentFootball = Footballs.findOne({_id:currentFootballId});
            var existingValuations = currentFootball.footballValuations;
            ...
            if(existingValuationsEmpty) {
                ...
                Valuations.update({_id:existingId},{$set:{valuationSelections: selections}});
                return existingId;
            } else {
        ...
        var valuationId = getValuation(valuation);
        ...
        var currentActivated = Footballs.findOne({_id:currentFootballId}).footballActivated;
        if(currentActivated == false) {
            Footballs.update({_id:currentFootballId},{$set:{footballActivated:activated}});
        }
        ...
        if(valuationsCount == 1) {
            if(!selections || selections.length == 0) {
                ...
                if(current == false && trading == false) {
                    ...
                    Footballs.update({_id: currentFootballId}, {$set: {footballActivated: false}});
                }
            }
        } else {
            if(!selections || selections.length == 0) {
    ...
    valuationBuildSelect: function(currentValuationId, selection, footballId) {
        ...
        switch(footballType) {
            case "market":
                switch(selection) {
                    case "EV/Revenue (FY0)":
                        return Valuations.update(currentValuationId, {$set: {valuationMetric:"EV/Revenue",valuationPeriod:"FY0",valuationOutput:"EV/Revenue",valuationOutputPeriod:"FY0"}});
                        break;
                    case "EV/Revenue (LTM)":
                    ...
                    case "EV/EBITDA (FY2)":
                        ...
                        break;
                    case "Price/Earnings (LTM)":
                        return Valuations.update(currentValuationId, {$set: {valuationMetric:"Price/Earnings",valuationPeriod:"LTM",valuationOutput:"Price/Earnings",valuationOutputPeriod:"LTM"}});
                        break;
                    case "Price/Earnings (FY1)":
                        return Valuations.update(currentValuationId, {$set: {valuationMetric:"Price/Earnings",valuationPeriod:"FY1",valuationOutput:"Price/Earnings",valuationOutputPeriod:"FY1"}});
                        break;
                    case "Price/Earnings (FY2)":
                        return Valuations.update(currentValuationId, {$set: {valuationMetric:"Price/Earnings",valuationPeriod:"FY2",valuationOutput:"Price/Earnings",valuationOutputPeriod:"FY2"}});
                        break;
                    case "EV/Attendance (FY0)":
                        return Valuations.update(currentValuationId, {$set: {valuationMetric:"EV/Attendance",valuationPeriod:"FY0",valuationOutput:"EV/Attendance",valuationOutputPeriod:"FY0"}});
                        break;
                }
                break;
            case "target":
    ...
    //},
    //valuationActiveUpdate: function(footballId, valuationId) {
    //    check(footballId, String);
    //    check(valuationId, String);
    //
    //    var active = getResultValue(footballId, valuationId);
    //    Valuations.update({_id:valuationId},{$set:{valuationActive:active}});
    }
});